      ...
      <Band className="!pb-8">
        <Shell>
          <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_380px]">
            <div>
              <Label>{branch.type === 'pusat' ? 'Kantor pusat' : 'Kantor cabang'}</Label>
              ...
              <dl className="mt-8 space-y-5">
                <div className="flex gap-3">
                  ...
                  <div>
                    <dt className="text-[13px] font-semibold text-ink-900">Alamat lengkap</dt>
                ...
                {branch.phone ? (
                  <div className="flex gap-3">
                    ...
                    <div>
                      <dt className="text-[13px] font-semibold text-ink-900">Telepon</dt>
                      <dd className="mt-0.5">
                ...
                <div className="flex gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <dt className="text-[13px] font-semibold text-ink-900">Jam operasional</dt>
                    <dd className="mt-1.5">
                      <OpenBadge branch={branch} />
                      <ul className="tnum mt-3 max-w-xs space-y-1 text-sm text-ink-600">
                        {[1, 2, 3, 4, 5, 6, 0].map((day) => {
                          const h = branch.hours.find((x) => x.day === day)
                          return (
                            <li key={day} className="flex justify-between gap-4 border-b border-line pb-1">
                              <span>{DAY_NAMES_ID[day]}</span>
                              <span className={h?.opensAt ? 'font-medium text-ink-900' : 'text-ink-400'}>
            ...
            <div>
              <Card className="overflow-hidden">
                ...
                />
                <div className="p-5">
                  <p className="text-[13px] font-semibold text-ink-900">Koordinat</p>
                  ...
                  <Action href={directionsLink(branch.latitude, branch.longitude, branch.name)} external className="mt-4 w-full">
                    Buka di aplikasi peta <Icon.arrow className="size-4" />
                  </Action>
                </div>
              </Card>
            </div>
        ...
        <Band>
          <Shell>
            ...
            <ul className="grid gap-5 sm:grid-cols-2">
              {others.map((b) => (
                <Card as="li" key={b.id} hover className="p-5">
                  <h3 className="t-h3">
                    <a href={`/lokasi/${b.slug}`} className="hover:text-green-700 hover:underline">{b.name}</a>
                  </h3>
                  <p className="mt-1 text-sm leading-relaxed text-ink-600">{b.address}</p>
                  <div className="mt-3">
                    <OpenBadge branch={b} />
                  </div>
                </Card>
